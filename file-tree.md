...
- `gate_a_sun_longitude.dart` — **第三方尺子**：Meeus 太阳视黄经（章动+光行差）→ 交节秒级时刻
- `gate_a_verify_astronomy.dart` — 全年 24 节气「数据包 vs 天算」逐项比对
...
- 原 GA-3 用 `-1min / -30s` 作「分钟精度是否足够」的判据 —— **该判据无效**：
  ...
  而实测 2026 十二「节」中只有 2 个与天算同分钟，差异窗口 **16s ～ 729s**。
- 改为：先算**差异窗口** `[min(数据包, 天算), max(数据包, 天算))`，
  再在「窗口起点 −1s / 窗口起点 / 数据包边界 −1s / 数据包边界」四点做人工对照。
- Gate 判据拆成两层：`Gate A1 PROFESSIONAL SOFTWARE COMPATIBILITY`
  （专业软件与卦眼是否一致）与 `Gate A2 SOLAR TERM ABSOLUTE PRECISION`
  （数据源绝对精度到何量级，只能如实记录）。
- 天算尺子**不入产品、不改架构**：它只用于测量窗口；
  真要升级也只是换 `assets/calendar/*.json` 数据包，`CalendarEngine` 一行不改。
- 期间修掉尺子自身两个 bug 并沉淀为哨兵测试：JD↔Unix 纪元多加 0.5 天（全体偏移 12 小时）、
  章动缺项导致数角秒残差。

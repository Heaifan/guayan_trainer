...
class DslExprFormatter {
  static String formatExpr(RuleExpr expr) {
    ...
    if (expr is PredicateExpr) {
      ...
      if (opId == 'chu_mu') {
        return '${_b(ops[0])} 出库于 ${_b(ops[1])} 冲 ${_b(ops[2])}';
      }
      if (opId == 'ru_mu') return '${_b(ops[0])} 入库于 ${_b(ops[1])}';
      if (opId == 'chong_mu') return '${_b(ops[0])} 冲库于 ${_b(ops[1])}';
      ...
      if (opId == 'in_tomb') return '${_b(ops[0])} 在库';
      if (opId == 'stem_is') return '${_b(ops[0])} 天干为 ${_l(ops[1])}';
      if (opId == 'branch_is') return '${_b(ops[0])} 地支为 ${_l(ops[1])}';
      if (opId == 'element_is') return '${_b(ops[0])} 五行为 ${_l(ops[1])}';
      if (opId == 'wuxing_overcomes') return '${_b(ops[0])} 克 ${_b(ops[1])}';
      if (opId == 'branch_clashes') return '${_b(ops[0])} 冲 ${_b(ops[1])}';
      if (opId == 'branch_combines') return '${_b(ops[0])} 合 ${_b(ops[1])}';
      if (opId == 'branch_punishes') return '${_b(ops[0])} 刑 ${_b(ops[1])}';
      if (opId == 'branch_harms') return '${_b(ops[0])} 害 ${_b(ops[1])}';
      if (opId == 'branch_breaks') return '${_b(ops[0])} 破 ${_b(ops[1])}';
    }
    return '';
  }

...
class PredicateEvaluator {
  ...
  PredicateResult evaluate(RuleExpr expr, BindingContext bindingContext, FactSnapshot snapshot) {
    if (expr is AllExpr) {
      ...
      if (!childResult.matched) {
        return PredicateResult.match(const []); // Negative provenance, no evidence
      }
      return PredicateResult.fail;
  ...
  PredicateResult _evaluatePredicate(PredicateExpr expr, BindingContext bindingContext, FactSnapshot snapshot) {
    final operatorImpl = registry.getOperator(expr.operatorId);
    if (operatorImpl == null) {
      throw EvaluationException('Operator implementation not found: ${expr.operatorId}');
    }
    ...
    for (final op in expr.operands) {
      if (op is BindingRefOperand) {
        ...
        if (ref == null) {
          throw EvaluationException('Unresolved binding reference in predicate: ${op.bindingName}');
        }
        ...
        throw EvaluationException('Unsupported operand type: ${op.runtimeType}');
      }
    }

    final context = OperatorContext(snapshot: snapshot, bindingContext: bindingContext);

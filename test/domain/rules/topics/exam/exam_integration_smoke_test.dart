...
void main() {
  group('R5-F Round 2: Integration Smoke', () {
    test(
      'line/2 fuMu + xun_kong yields common and exam tags deterministically',
      () {
        ...
        final hitXunKong = result1.ruleHits.any(
          (h) => h.ruleId.id == 'exam.line.2.state.fu_mu_xun_kong',
        );
        expect(hitXunKong, isTrue);

        final examEvidence = result1.ruleHits
            .firstWhere(
              (h) => h.ruleId.id == 'exam.line.2.state.fu_mu_xun_kong',
            )
            .supports;
        ...
        expect(tagStrs2, equals(tagStrs));

        final hits1 = result1.ruleHits.map((h) => h.hitId.id).toList()..sort();
        final hits2 = result2.ruleHits.map((h) => h.hitId.id).toList()..sort();
        expect(hits2, equals(hits1));

        final ev1 = result1.evidenceNodes.map((n) => n.id.id).toList()..sort();
        final ev2 = result2.evidenceNodes.map((n) => n.id.id).toList()..sort();
        expect(ev2, equals(ev1));
      },
    );
  });
}

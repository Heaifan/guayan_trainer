  ...
  group('Canonical Operators', () {
    ...
    setUp(() {
      ...
      snapshot = FactSnapshot.build([
        ...
        FactRecord(factId: 'f3', subject: refA, predicateId: 'nayin', value: RuleValue.string('tian_he_shui'), origin: FactOrigin.baseRelation),
        ...
        FactRecord(factId: 'f8', subject: refM, predicateId: 'relation.generate', value: RuleValue.string('line/A'), origin: FactOrigin.baseRelation),
        FactRecord(factId: 'f9', subject: refA, predicateId: 'relation.ru_mu', value: RuleValue.string('tomb/T'), origin: FactOrigin.baseRelation),
        FactRecord(factId: 'f10', subject: refD, predicateId: 'relation.chong_mu', value: RuleValue.string('tomb/T'), origin: FactOrigin.baseRelation),
        FactRecord(factId: 'f11', subject: refA, predicateId: 'relation.chu_mu', value: RuleValue.string('tomb/T,day/D'), origin: FactOrigin.baseRelation),
    ...
    test('relative', () {
      final res = eval(ConditionId.relative, [ResolvedOperand.ref(refA), ResolvedOperand.literal(RuleValue.string('parent'))]);
      expect(res.matched, true);
    });

    test('spirit', () {
      final res = eval(ConditionId.spirit, [ResolvedOperand.ref(refA), ResolvedOperand.literal(RuleValue.string('azureDragon'))]);
      expect(res.matched, true);
    });

    test('nayin_is', () {
      final res = eval(ConditionId.nayinIs, [ResolvedOperand.ref(refA), ResolvedOperand.literal(RuleValue.string('tian_he_shui'))]);
      expect(res.matched, true);
    });
    ...
    test('generate', () {
      final res = eval(ConditionId.generate, [ResolvedOperand.ref(refM), ResolvedOperand.ref(refA)]);
      expect(res.matched, true);
    });

    test('ru_mu', () {
      final res = eval(ConditionId.ruMu, [ResolvedOperand.ref(refA), ResolvedOperand.ref(refT)]);
      expect(res.matched, true);
    });

    test('chong_mu', () {
      final res = eval(ConditionId.chongMu, [ResolvedOperand.ref(refD), ResolvedOperand.ref(refT)]);
      expect(res.matched, true);
    });

    test('chu_mu', () {
      final res = eval(ConditionId.chuMu, [ResolvedOperand.ref(refA), ResolvedOperand.ref(refT), ResolvedOperand.ref(refD)]);
      expect(res.matched, true);
    });

    test('has_tag', () {
      final res = eval(ConditionId.hasTag, [ResolvedOperand.ref(refA), ResolvedOperand.literal(RuleValue.string('shensha')), ResolvedOperand.literal(RuleValue.string('shensha.custom.foo'))]);
      expect(res.matched, true);
    });
  });
}
